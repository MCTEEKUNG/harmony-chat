...
/**
 * Full-screen User Settings overlay. Rendered on top of the app (not a route)
 * so the voice connection underneath stays alive while settings are open.
 */
export default function UserSettings({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-d-mid animate-fade-in md:flex-row">
      <aside className="d-scroll shrink-0 overflow-y-auto bg-d-darkest p-3 md:w-64 md:p-4">
        <p className="mb-2 px-3 text-xs font-bold uppercase tracking-wide text-d-muted">User Settings</p>
      ...
      <main className="d-scroll relative flex-1 overflow-y-auto px-5 py-8 md:px-12">
        ...
        {dirty && (
          <div className="fixed bottom-4 left-1/2 z-50 flex w-[min(92vw,560px)] -translate-x-1/2 items-center gap-3 rounded-lg bg-d-darkest px-4 py-3 shadow-2xl ring-1 ring-black/40 animate-fade-in">
            ...
            <button onClick={reset} className="rounded px-3 py-1.5 text-sm text-d-muted hover:text-d-text">
              Reset
            </button>
            <button
              onClick={save}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-d-muted">{label}</label>
      {children}
    </div>
  );
}
